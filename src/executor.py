...
class HardenedRiskEngine:
    ...
    """
    def __init__(self, simulated_balance: float = 10000.0, state_file: Optional[str] = DEFAULT_RISK_STATE_PATH):
...
class ExecutionModule:
    ...
    def __init__(self, exchange_id='mock', paper_trade=True, state_file: Optional[str] = DEFAULT_RISK_STATE_PATH):
    ...
    def process_signal(self, symbol: str, action: str, entry_price: float, defensive_sl: float, signal_id: Optional[str] = None) -> dict:
        approved, qty, reason = self.risk.validate_and_size(symbol, action, entry_price, defensive_sl, signal_id)
        if not approved:
        ...
        self.risk.register_entry(symbol, action, qty, entry_price, defensive_sl, tp)
        return {
            "approved": True,
            "reason": "Approved",
            "symbol": symbol,
